...
def recognize_faces(stop=False):
    cap = cv2.VideoCapture(0)
    if stop:
        try:
            cap.release()   
            ...
            return 'Error: Unable to stop the attendance.'
    try:
        if not cap.isOpened():
        ...
        start_time = datetime.now()
        while (datetime.now() - start_time).total_seconds() < settings.FACE_RECOGNITION_TIMEOUT or 120:
            print(res)
            ret, frame = cap.read()
            if not ret:
            ...
            for roll_number, embed, name in embeddings:
                ...
                ''' Below code is for checking the status of the student , if he is not present
                then it will mark his status as False.'''
                # else:
                #     for x in res:
                #         if x['roll_no'] == roll_number:
                #             x['status'] = False
...
def set_attendance(force=False, stop=False):
    if stop:
        return recognize_faces(stop=True)
    day = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
    current_day_index = timezone.now().weekday()
